...
function SWEP:PrimaryAttack()
    ...
    if canLockpick ~= true and (
            ...
            (not ent:isDoor() and not ent:IsVehicle() and not string.find(string.lower(ent:GetClass()), "vehicle") or not ent.isFadingDoor))
        ) then
        return
    end
    ...
    local endDelta = hook.Call("lockpickTime", nil, Owner, ent)
...
function SWEP:DrawHUD()
    ...
    draw.RoundedBox(8, x, y, width, height, Color(135, 206, 235,120))
